...
double height_rate(InputMapper &mapper)
{
    communication::msg::MotionCommands message;
    mapper.fill_message(message);
    return message.vel_des.z;
}
...
void test_default_trigger_height_rate_and_homie_chord()
{
    const RemoteConfig config = remote_controller::load_remote_config(
        REMOTE_CONTROLLER_TEST_CONFIG_PATH);
    ...
    {
        InputMapper mapper(config);
        mapper.set_signals({{"js.axis.5", -1.0}, {"js.axis.4", -1.0}});
        expect_close(height_rate(mapper), 0.0);
    }

    {
        InputMapper mapper(config);
        mapper.set_signals({{"js.axis.5", 1.0}, {"js.axis.4", -1.0}});
        expect_close(height_rate(mapper), -1.0);
    }
...
int main()
{
    ...
    test_default_trigger_height_rate_and_homie_chord();
