...
class RobotControlFramework:
    ...
    def _set_observation(self, observation: RobotObservation) -> None:
        ...
        self.current_raw_height_rate = self._finite_scalar(
            observation.raw_height_rate,
            "raw_height_rate",
        )
    ...
    @staticmethod
    def _finite_scalar(source: object, name: str) -> float:
        array = np.asarray(source)
        is_real_number = np.issubdtype(array.dtype, np.number) and not np.issubdtype(
            array.dtype,
            np.complexfloating,
        )
        if array.shape != () or not is_real_number:
            raise ValueError(
                f"robot observation {name} must be a finite scalar"
            )
        value = float(array)
