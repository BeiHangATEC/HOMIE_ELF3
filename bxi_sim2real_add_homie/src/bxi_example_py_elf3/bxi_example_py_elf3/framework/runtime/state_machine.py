...
class RobotStateMachine:
    ...
    def _update_active_transition(self, dt: float) -> None:
        ...
        if self._active is not active:
            # A sampled state synchronously replaced this transition. Re-apply
            # the replacement path so the cancelled session cannot own the
            # final motor target for this control cycle.
            if self._active is None:
                self.current.on_update(self._ctx, 0.0)
            else:
                self._update_active_transition(0.0)
            return
        if finished:
